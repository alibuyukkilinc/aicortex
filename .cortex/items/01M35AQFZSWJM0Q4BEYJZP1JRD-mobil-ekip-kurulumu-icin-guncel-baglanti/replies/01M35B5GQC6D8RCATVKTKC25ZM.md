...
1. **Uzak MCP adresi ve kimlik doğrulama:** `POST <sunucu>/mcp/p/<proje>`, başlık `Authorization: Bearer <ajan tokenı>`. Akış destekli HTTP, durumsuz. Bu repoda proje kimliği `cortex`. stdio köprüsü (`cortexboard mcp --hub <url> --project <id> --token <t>`) da çalışmaya devam ediyor.
