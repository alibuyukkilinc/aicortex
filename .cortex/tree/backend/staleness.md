...
summary: "Düğümler koda bağlanır, yazılınca ya da onaylanınca commit'e
  sabitlenir. Kod sonradan değişirse eskir: yüksek (bağlı satır, silme, taşıma),
  orta (dosyanın başka yeri), düşük (yalnızca biçim, projenin Prettier'ına
  sorulur). Yalnızca yüksek ve orta sayılır. Ertelenebilir."
links:
  code:
    - file: src/git/git.ts
